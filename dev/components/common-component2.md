...
VitePress也能做导航，[@茂茂大神](https://github.com/maomao1996/mm-notes)
...
import MNavLinks from './components/MNavLinks.vue'  <-- copy
...
export default {
  ...
  enhanceApp({app}) {
    // 注册组件
    app.component('MNavLinks' , MNavLinks)    <-- copy
  },
}
...
::: danger 这个地方在进行dev时，会一个错误
 - 将以下代码复制到`config.mjs` `defineConfig`这个里边
...
在 .vitepress/theme/utils 目录新建 data.ts 文件
...
<style src="/.vitepress/theme/style/nav.scss"></style>   < ---cpoy

<script setup>                                             < ---cpoy
import { NAV_DATA } from '/.vitepress/theme/untils/data'    < ---cpoy
</script>                                                    < ---cpoy


# 我的导航

<MNavLinks v-for="{title, items} in NAV_DATA" :title="title" :items="items"/>  < ---cpoy
